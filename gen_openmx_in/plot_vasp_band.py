...
#                and plot the band structure use the vasp EIGENVAL file.
#              ::Input File::
#                - EIGENVAL
#                - KPOINTS
#                - OUTCAR
...
def main(argv):
    ...
    # | Band Data Read In |
    # +-------------------+
    ## Check the existance of the EIGENVAL, KPOINTS and OUTCAR. 
    if not os.path.isfile('EIGENVAL'):
        print("[error] EIGENVAL not found!!!")
        sys.exit(0)
    elif not os.path.isfile('KPOINTS'):
        print("[error] KPOINTS not found!!!")
        sys.exit(0)
    elif not os.path.isfile('OUTCAR'):
    ...
    rlv = np.zeros((3, 3))
    find_rlv_block = False
    rlv_index = -1
    fermi_energy = -233.666
    with open("OUTCAR", 'r') as frp:
        for line in frp:
            if 'ISPIN' in line:
    ...
    with open("KPOINTS", 'r') as frp:
        file_lines = frp.readlines()
    kpoints_quantity_each_kpath = int(file_lines[1].split()[0])
    hsk_quantity = 0 # High Symmetic Kpoint quantity
    for line in file_lines:
        if ('0.' in line) and (len(line.split()) == 4):
            hsk_quantity += 1
    ...
    with open ("EIGENVAL", 'r') as frp:
        file_lines = frp.readlines()
    check_kpoints_quantity = int(file_lines[5].split()[1])
    if check_kpoints_quantity != kpoints_quantity:
        print('[error] EIGENVAL kpoints quantity do not match with KPOINTS!!!')
        sys.exit(1)
    band_quantity_each_spin = int(file_lines[5].split()[2])
    band_quantity  = band_quantity_each_spin * spin_num
    # Read band data
    ...
    for kpoints_index in range(kpoints_quantity):
        line = file_lines[read_line_index].split()
        ...
        for band_index in range(band_quantity_each_spin):
            line = file_lines[read_line_index].split()
            spin_up_band_data[band_index, kpoints_index] = float(line[1])
            if len(line) == 3: # ISPIN == 1
                ...
                if spin_num != 2:
                    print("[error] ISPIN do not agree with EIGENVAL!!!")
                    sys.exit(2)
                spin_down_band_data[band_index, kpoints_index] = float(line[2])
            else:
    ...
    # Combine the band
    if spin_num == 1:
        band_data = spin_up_band_data
    else:
        band_data = np.concatenate((spin_up_band_data, spin_down_band_data),
                                   axis=0)
    sorted_band_data = band_data_sort(band_data)
    #
    # Summary [Part I] #
    ...
    # - The number of kpoints in each k-path      :: kpoints_quantity_each_kpath
    ...
    for kpath_index in range(kpath_quantity):
        ...
        hsk_distance_list[kpath_index] = \
            cal_k_distance(rlv, start_hsk, end_hsk)
    ...
    for kpath_index in range(kpath_quantity):
        ...
        for kpath_kpoints_index in range(kpoints_quantity_each_kpath):
            kpoints_index += 1
            start_hsk = kpath_vector_list[kpath_index][0]
    ...
    # If is a conductor
        ...
        for band_index in range(band_quantity):
            if (fermi_energy <= band_k_average[band_index]):
                ...
                break
    # Translate the Energy to E_f = 0
    sorted_band_data -= fermi_energy
    ...
    # - Kpoints Corrdinate list                 :: kpoints_corrdinate_list
