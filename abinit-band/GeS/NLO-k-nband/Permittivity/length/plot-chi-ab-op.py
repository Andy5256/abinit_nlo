...
info1 = np.loadtxt('Im_chi_11_k'+str(k)+'.dat')
ax1.plot(info1[:,0], info1[:,1], label='openmx'+str(k)+'k140bs_0.02')
...
ax2 = plt.subplot(122)
info2 = np.loadtxt('Re_chi_11_k'+str(k)+'.dat')
ax2.plot(info2[:,0], shift + info2[:,1], label='openmx'+str(k)+'k140bs_0.02+'+str(shift))
abinit = np.loadtxt('optic_2_0001_0001-linopt'+b+'.out')
...
ax2.legend()
plt.tight_layout()
plt.savefig('Chi_11_'+b+str(k)+'k_'+str(shift)+'.png')
